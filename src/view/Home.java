...
public class Home extends MyFrame {
    ...
    private SyncedList<Notification> notifications;
    ...
    private void fill() {
        ...
        notifications.setQuery(x -> x.getUserId() == current.getId() && !x.isRead());
    }
    ...
    private void createMenu() {
        ...
        ////////NOTIFICATIONS/////////
        notificationMenu = new JMenu("Notificações");

        notifications = new SyncedList<>(Notification.store);

        notifications.onLoad.addListener(e -> {
            fillNotifications();
        });
    ...
    private SimpleDateFormat notificationdateFormat = new SimpleDateFormat("dd/MM/yyyy");
    ...
    private void fillNotifications() {
        ...
        if (size > 0) {
            notificationMenu.setText("Notificações (" + size + ")");

            for (Notification n : notifications) {
                JMenuItem menuItem = new JMenuItem(new AbstractAction(notificationdateFormat.format(n.getDate()) + " - " + n.getText()) {
                    public void actionPerformed(ActionEvent ae) {
                        n.setRead(true);
            ...
            JMenuItem menuItem = new JMenuItem(new AbstractAction("Sem notificações") {
                public void actionPerformed(ActionEvent ae) {
                }
            });
